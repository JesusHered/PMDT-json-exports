...
$newLocationFolderPath = "$($contentDataBaseFolderPath)/Locations_230105/Tier2-3"
$defaultLocationPath = "$($newLocationFolderPath)/Default Tier2-3 Template"
$oldLocationFolderPath = "$($contentDataBaseFolderPath)/NewLocations/Tier2-3"
        ...
        foreach($newLocation in $newLocations.Children)
        {
            ...
            foreach ($row in $jsonData) {
                ...
                if($row.url -eq $urlPath)
                {
                    ...
                    $pageItem = Get-Item -Path "$($database):$newPath/ContentBlocks/Services"
                    $pageItem.Editing.BeginEdit()
                    $services = "<ul>"
