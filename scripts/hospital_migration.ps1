...
$newLocationFolderPath = "$($contentDataBaseFolderPath)/Locations_230105/Hospital"
$defaultLocationPath = "$($newLocationFolderPath)/Default Hospital Template"
$oldLocationFolderPath = "$($contentDataBaseFolderPath)/NewLocations/Hospital"
...
function Create-LocationStructureFromJson
{
    Process
    {
        ...
        $jsonData = $jsonData =  curl 'https://raw.githubusercontent.com/LuigiEspinosa/promedica-scraper/main/json/ProMedica/locations/locations-details.json' -UseBasicParsing
        ...
        foreach($newLocation in $newLocations.Children)
        {
            ...
            $newPathItemPage = "$($database):$newPath/Landing Page/Page"
            ...
            foreach ($row in $jsonData) {
                ...
                if($row.url -eq $urlPath)
                {
                    ...
                    $pageItem = Get-Item -Path “$($database):$newPath/Landing Page/Content/Tabbed Content/Services”
                    $pageItem.Editing.BeginEdit()
                    $pageItem[“Content”] = “”;
                    foreach($service in $row.content.services){
                        $serviceId = Verif-ServiceExist -ServiceName $service
                        if (![string]::IsNullOrEmpty($pageItem[“Content”])) {
                            $pageItem[“Content”] += “|”
                        }
                        $pageItem[“Content”] += $serviceId
                    }
                    $pageItem.Editing.EndEdit()
                    Write-Host "    Services item was update with JSON DATA"
                    
                    $pageItem = Get-Item -Path “$($database):$newPath/Landing Page/Content/Tabbed Content/Amenities"
                    $pageItem.Editing.BeginEdit()
                    $pageItem[“Content”] = “”;
                    foreach($amenitie in $row.content.amenities){
                        $amenitieId = Verif-AmenitiesExist -AmenitieName $amenitie
                        if (![string]::IsNullOrEmpty($pageItem[“Content”])) {
                            $pageItem[“Content”] += “|”
                        }
                        $pageItem[“Content”] += $amenitieId
                    }
                    $pageItem.Editing.EndEdit()
                    Write-Host "    Amenities item was update with JSON DATA"
                }
            }
        }
    }
